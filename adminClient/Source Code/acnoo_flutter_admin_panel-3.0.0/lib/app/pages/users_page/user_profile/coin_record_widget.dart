...
class CoinRecordWidget extends StatefulWidget {
  ...
  const CoinRecordWidget({super.key, required this.userId});
  final int userId;
  @override
  State<CoinRecordWidget> createState() => _CoinRecordState();
}

class _CoinRecordState extends State<CoinRecordWidget>{
  List<CoinRecord> coinRecordList = [];
  ...
  Future<void> getCoinRecordList(BuildContext context) async {
    ...
    try{
      ...
      list = await currencyRecordService.getCoinRecordList(param);
    } on DioError catch(e){
  ...
  Future<void> getCoinRecordListCount(BuildContext context) async {
    ...
    try{
      CurrencyRecordSearchParam param = CurrencyRecordSearchParam(widget.userId, changeType, _startDateController.text, _endDateController.text, _currentPage + 1, _rowsPerPage);
      count = await currencyRecordService.getCoinRecordListCount(param);
    } on DioError catch(e){
      ErrorCode errorCode = ErrorCode.fromJson(e.response?.data, e.response?.statusCode);
      ErrorDialog.showError(context, errorCode);
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: _sizeInfo.padding,
                            child: Text(
                              '${l.S.of(context).showing} ${_currentPage * _rowsPerPage + 1} ${l.S.of(context).to} ${_currentPage * _rowsPerPage + coinRecordList.length} ${l.S.of(context).OF} ${coinRecordList.length} ${l.S.of(context).entries}',
