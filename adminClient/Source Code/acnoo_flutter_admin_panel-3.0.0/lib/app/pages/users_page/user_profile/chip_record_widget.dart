...
class ChipRecordWidget extends StatefulWidget {
  ...
  const ChipRecordWidget({super.key, required this.userId});
  final int userId;
  @override
  State<ChipRecordWidget> createState() => _ChipRecordState();
}

class _ChipRecordState extends State<ChipRecordWidget>{
  List<ChipRecord> chipRecordList = [];
  ...
  Future<void> getChipRecordList(BuildContext context) async {
    ...
    try{
      ...
      list = await currencyRecordService.getChipRecordList(param);
    } on DioError catch(e){
  ...
  Future<void> getChipRecordListCount(BuildContext context) async {
    ...
    try{
      CurrencyRecordSearchParam param = CurrencyRecordSearchParam(widget.userId, changeType, _startDateController.text, _endDateController.text, _currentPage + 1, _rowsPerPage);
      count = await currencyRecordService.getChipRecordListCount(param);
    } on DioError catch(e){
      ErrorCode errorCode = ErrorCode.fromJson(e.response?.data, e.response?.statusCode);
      ErrorDialog.showError(context, errorCode);
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: _sizeInfo.padding,
                            child: Text(
                              '${l.S.of(context).showing} ${_currentPage * _rowsPerPage + 1} ${l.S.of(context).to} ${_currentPage * _rowsPerPage + chipRecordList.length} ${l.S.of(context).OF} ${chipRecordList.length} ${l.S.of(context).entries}',
