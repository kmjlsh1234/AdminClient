...
import 'package:acnoo_flutter_admin_panel/app/services/admin_user/drop_out_user_service.dart';
...
class DropOutUsersListView extends StatefulWidget {
  const DropOutUsersListView({super.key});

  @override
  State<DropOutUsersListView> createState() => _DropOutUsersListViewState();
}

class _DropOutUsersListViewState extends State<DropOutUsersListView> {
  ///_____________________________________________________________________Variables_______________________________
  List<DropOutUser> dropOutUserList = [];
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _startDateController = TextEditingController();
  final TextEditingController _endDateController = TextEditingController();
  final DropOutUserService dropOutUserService = DropOutUserService();
  final String detailRoute = "/users/user/details";
  ...
  Future<void> getAdminList(BuildContext context) async{
    ...
    try{
      ...
      DropOutUserSearchParam dropOutUserSearchParam = DropOutUserSearchParam(searchType, _searchQuery, _startDateController.text,_endDateController.text,_currentPage + 1, _rowsPerPage);
  ...
  Future<void> getAdminListCount(BuildContext context) async{
    ...
    try{
      setState(() => isLoading = true);
      DropOutUserSearchParam dropOutUserSearchParam = DropOutUserSearchParam(searchType, _searchQuery, _startDateController.text,_endDateController.text,_currentPage + 1, _rowsPerPage);
      count = await dropOutUserService.getAdminUserListCount(dropOutUserSearchParam);
    }on DioError catch(e){
      ErrorCode errorCode = ErrorCode.fromJson(e.response?.data, e.response?.statusCode);
      ErrorDialog.showError(context, errorCode);
    }
  ...
  Future<void> getAdminInfo(BuildContext context, int userId) async {
    try{
      //AdminUserDetail adminUserDetail = await dropOutUserService.getAdminUser(userId);
      //context.push(detailRoute ?? '/404', extra: adminUserDetail,);
    } on DioError catch(e){
      ErrorCode errorCode = ErrorCode.fromJson(e.response?.data, e.response?.statusCode);
      ErrorDialog.showError(context, errorCode);
    }
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Padding(
                                  padding: _sizeInfo.padding,
                                  child: Text(
                                    '${l.S.of(context).showing} ${_currentPage * _rowsPerPage + 1} ${l.S.of(context).to} ${_currentPage * _rowsPerPage + dropOutUserList.length} ${l.S.of(context).OF} ${dropOutUserList.length} ${l.S.of(context).entries}',
  ...
  Container showingSearchTypeDropDown(
    ...
    return Container(
      ...
      child: DropdownButtonFormField2<String>(
        ...
        isExpanded: true,
        value: searchType,
        items: ["EMAIL", "MOBILE"].map((String value) {
                      ...
                      switch (action) {
                        case 'Edit':
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                                content: Text('${lang.edit} ${data.email}')),
                          );
                          break;
                        case 'View':
                          getAdminInfo(context, data.userId);
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                                content:
                                    Text('${lang.viewed} ${data.email}')),
                          );
                          break;
                        case 'Delete':
                          setState(() {
                            //users.remove(data);
                            dropOutUserList.remove(data);
                          });
                          break;
                      }
                    },
